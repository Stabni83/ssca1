...
def create_mel_filterbank(sr, n_fft, n_mels=150):
    """Create Mel filterbank matrix for frequency compression."""
    return librosa.filters.mel(sr=sr, n_fft=n_fft, n_mels=n_mels, fmin=0, fmax=sr / 2)


def compress_to_mel(X_stft, mel_fb):
    """Compress STFT magnitude spectra to Mel scale using filterbank."""
    X_mel = []
...
def reconstruct_from_mel(S_mel, X_stft, mel_fb, p=2.0):
    """Reconstruct masks from Mel to linear frequency scale."""
    X_mix = X_stft[0]
    X_phase = np.angle(X_mix)
    N, F_mel, T = S_mel.shape

    # Calculate Wiener masks in Mel scale
    mag_sources_mel = np.abs(S_mel) ** p
    denom = np.sum(mag_sources_mel, axis=0, keepdims=True) + 1e-8
    masks_mel = mag_sources_mel / denom

    # Reconstruct masks to linear frequency scale
    masks_linear = np.array([mel_fb.T @ masks_mel[n] for n in range(N)])
